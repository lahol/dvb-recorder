...
typedef struct _UiEpgEventDetailPrivate {
    /* private data */
    GtkWidget *title_label;
    GtkWidget *description;

    EPGEvent *event;
} UiEpgEventDetailPrivate;

G_DEFINE_TYPE_WITH_PRIVATE(UiEpgEventDetail, ui_epg_event_detail, GTK_TYPE_BIN);
...
static void populate_widget(UiEpgEventDetail *self)
{
    ...
    gtk_widget_set_halign(priv->title_label, GTK_ALIGN_START);

    priv->description = gtk_text_view_new();
    gtk_text_view_set_editable(GTK_TEXT_VIEW(priv->description), FALSE);
    gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(priv->description), GTK_WRAP_WORD_CHAR);
    gtk_text_view_set_justification(GTK_TEXT_VIEW(priv->description), GTK_JUSTIFY_FILL);
    gtk_text_view_set_left_margin(GTK_TEXT_VIEW(priv->description), 5);
    gtk_text_view_set_right_margin(GTK_TEXT_VIEW(priv->description), 13);
#if GTK_CHECK_VERSION(3, 18, 0)
    gtk_text_view_set_top_margin(GTK_TEXT_VIEW(priv->description), 2);
    gtk_text_view_set_bottom_margin(GTK_TEXT_VIEW(priv->description), 2);
#endif
    ...
    gtk_box_pack_start(GTK_BOX(desc_box), priv->description, TRUE, TRUE, 0);
...
void ui_epg_event_detail_set_event(UiEpgEventDetail *self, EPGEvent *event)
{
    ...
    if (event) {
        ...
        gtk_label_set_markup(GTK_LABEL(priv->title_label), text);
        g_free(text);

        buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(priv->description));
